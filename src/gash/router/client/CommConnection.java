...
public class CommConnection {
	...
	// protected static AtomicReference<CommConnection> instance = new
	// AtomicReference<CommConnection>();
	private MessageClient selfClient;
	private String host;
	private int port;
	private ChannelFuture channel; // do not use directly call
									// connect()!
	private CommConnection self;
	 ...
	 * @param port
	 */
	protected CommConnection(String host, int port, MessageClient mc) {
		this.host = host;
		this.port = port;
		this.selfClient = mc;
		init();
	...
	// MessageClient selfClient) {
	// instance.compareAndSet(null, new CommConnection(host, port, selfClient));
	// return instance.get();
	// }
	...
	private void init() {
		...
		try {
			CommInit si = new CommInit(false, selfClient);
