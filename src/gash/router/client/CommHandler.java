...
public class CommHandler extends SimpleChannelInboundHandler<CommandMessage> {
	...
	private volatile MessageClient selfClient;
	public CommHandler(MessageClient mc) {
		this.selfClient = mc;
	}
		...
		try {
			// TODO How can you implement this without if-else statements?
			 if (msg.hasLeaderStatus()) {
				 if(msg.getLeaderStatus().getAction()==LeaderQuery.THELEADERIS){
					 if(msg.getLeaderStatus().getState() != Election.LeaderStatus.LeaderState.LEADERUNKNOWN){
						 selfClient.returnLeaderId(msg);
					 }else{
						 System.out.println("Leader Unknown. Wait for leader election.");
						 return;
					 }
				 }
			}else if(msg.hasReponseMsg()){
				selfClient.serverResponseHandler(msg);				
			}
			else{
