...
public class MessageServer {
	 ...
	 */
	protected static class StartCommandCommunication implements Runnable {
		RoutingConf conf;
		RaftManager thisManager;

		public StartCommandCommunication(RoutingConf conf, StartWorkCommunication work) {
			this.conf = conf;
			thisManager = work.manager;
		}
		...
		public void run() {
			...
			try {
				...
				b.childHandler(new CommandInit(conf, compressComm, thisManager));
	 ...
	 *            The port to listen to
	 */
	protected static class StartWorkCommunication implements Runnable {
		ServerState state;
		RaftManager manager;
